export const img = { src: 'http://uat-apidb.kashinobi.com/home/public/img/page3/topBanner.png', altText: '是咖啡店，也是博物館' }
export const content = {
    tag: '文創商品,咖啡',
    title:
        '是咖啡店也是博物館！台南「文鼎留聲博物館」亮相：本事設計操刀一樓空間、首檔展覽開展',
    content: [
        `<img src='http://uat-apidb.kashinobi.com/home/public/img/page3/image1.png' alt="image1" />
    <br>
    <br>
    <br>
        <p>位於台南中西區的「文鼎留聲博物館」自2022年12月開幕，以「探索百年留聲時光」為概念，從音樂盒、留聲機、電子黑膠唱機、現代音訊等不同媒介，探索聲音留存的開始與形式，並結合科普知識與歷史介紹。透過「保留傳遞文化」、「美學教育」、「本土文化結合」、「文化承先啟後」四大理念，期盼可以推廣、延續新的音樂工藝文化。
        </p>
    <br>
    <br>
    <br>
    <img src='http://uat-apidb.kashinobi.com/home/public/img/page3/image2.png' alt="image2" /> 
    <br>
    <br>
    <br>
        <p>文鼎留聲博物館創辦人許文鼎先生表示，自身從14歲進入電器產業，曾推出風靡一時的國⺠唱機「達琳牌」；如今以畢生事業之根基⸺黑膠唱機為主題，希望建立博物館可以留下聲音的痕跡。
        <br>
        <br>
        一樓室內裝潢設計找來擅長以空間說故事的「本事空間製作所」操刀。過去知名作品包括台南天下大飯店、Paripari 咖啡店、詹記麻辣火鍋敦南店，以及近期推出的石門福華飯店皆出自於他們之手，此次本事設計則為台南文鼎留聲博物館打造是咖啡廳，也是博物館的復古空間。
        </p>
    <br>
    <br>
    <br>
    <img src='http://uat-apidb.kashinobi.com/home/public/img/page3/image3.png' alt="image3" /> 
    <br>
    <br>
    <br>
        <p>本次常設展將展出200年來不同的唱機工藝與留聲文物，分別以「古典廳」介紹自動演奏樂器的歷史發展與發聲原理，一窺19世紀前期的音樂娛樂；「留聲廳」紀錄留聲機的原理與發展歷程，探索『沒有電的時代，聲音如何被記錄？』；「現代廳」以電子唱機與現代音訊兩大主題，介紹走入現代後音訊設備的多元化發展。在串流音樂普及的現代社會當中，分享聲音紀錄的歷史文化與科普知識。
        <br>
        <br>
        文鼎留聲博物館創辦人許文鼎先生表示，自身從14歲進入電器產業，曾推出風靡一時的國⺠唱機「達琳牌」；如今以畢生事業之根基⸺黑膠唱機為主題，希望建立博物館可以留下聲音的痕跡。
        </p>
    <br>
    <br>
    <br>
    <img src='http://uat-apidb.kashinobi.com/home/public/img/page3/image4.png' alt="image4" /> `

    ],
    createDate: 'Sun Jan 01 2023 00:00:00 GMT+0800 (台北標準時間)'
}

export const tags = [
    '選物',
    '出國',
    '室內設計',
    '建築',
    '咖啡',
    '文創商品',
    '佳節精選',
]

export const popularContents = [
    { img: { src: 'http://uat-apidb.kashinobi.com/home/public/img/page3/extendReading_image.png', altText: '台南天下大飯店' }, title: `本事設計操刀！「台南天下大飯店」改裝搖身變：古銅曖昧藍與華麗摩登的時髦旅店！` },
    { img: { src: 'http://uat-apidb.kashinobi.com/home/public/img/page3/extendReading_image.png', altText: '台南天下大飯店' }, title: `本事設計操刀！「台南天下大飯店」改裝搖身變：古銅曖昧藍與華麗摩登的時髦旅店！` },
    { img: { src: 'http://uat-apidb.kashinobi.com/home/public/img/page3/extendReading_image.png', altText: '台南天下大飯店' }, title: `本事設計操刀！「台南天下大飯店」改裝搖身變：古銅曖昧藍與華麗摩登的時髦旅店！` },
]